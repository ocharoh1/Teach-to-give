...
function displayEvents(events) {
    ...
    events.forEach(event => {
        ...
        eventCard.innerHTML = `
            <img data-src="${event.imageUrl}" class="lazyload" alt="${event.title}">
            <h3>${event.title}</h3>
...
document.getElementById('applyFilters').addEventListener('click', () => {
    ...
    filteredEvents = eventsData.filter(event => {
        return (!filterPrice || event.price <= filterPrice) && 
...
function removeFromFavorites(eventId) {
    favorites = favorites.filter(e => e.id !== eventId);
    displayFavorites();
}
...
function displayFavorites() {
    ...
    favorites.forEach(event => {
        ...
        li.innerHTML = `${event.title} <button onclick="removeFromFavorites(${event.id})">Remove</button>`;
